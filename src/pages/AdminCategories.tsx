...
const AdminCategories: React.FC = () => {
  ...
  const categories = [
    { id: '1', name: 'PEINTURE & FINITION', icon: '🎨', products: 45, subcategories: 6, status: 'active' },
    { id: '2', name: 'COLLES & MASTICS', icon: '🧱', products: 38, subcategories: 8, status: 'active' },
    { id: '3', name: 'OUTILLAGE PEINTRE', icon: '🖌️', products: 28, subcategories: 6, status: 'active' },
    { id: '4', name: 'OUTILLAGE CARRELEUR', icon: '🔧', products: 24, subcategories: 6, status: 'active' },
    { id: '5', name: 'PRÉPARATION SOLS', icon: '🧱', products: 18, subcategories: 6, status: 'active' },
    { id: '6', name: 'FIXATION & VISSERIE', icon: '🔩', products: 32, subcategories: 6, status: 'inactive' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-6">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder={t('admin.categories.search') || 'Rechercher...'}
                value={searchQuery}
            ...
            <button className={`flex items-center gap-1.5 px-4 py-2.5 border ${borderClass} rounded-lg transition-colors ${textClass} ${hoverBorderClass}`}>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="text-3.5">{category.icon}</div>
                  <div className="flex-1">
                    <h3 className={`font-bold italic text-lg uppercase tracking-wide mb-1 ${textClass}`}>
                      {category.name}
                    </h3>
                    <p className={`text-xs ${mutedClass} mb-3`}>
                      {category.products} {t('catalogue.back').replace('← ', '')} • {category.subcategories} sous-catégories
                    </p>
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      ...
                      {category.status === 'active' ? t('admin.products.active') : t('admin.products.inactive')}
                    </span>
                  </div>
                </div>
