...
interface AdminSettingsBrandsCustomizationProps {
  isOpen: boolean;
  onClose: () => void;
}

const AdminSettingsBrandsCustomization: React.FC<AdminSettingsBrandsCustomizationProps> = ({
  isOpen,
  onClose
}) => {
  ...
  const loadBrandSettings = (brand: Brand) => {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      // Используем тот же формат что и в AdminBrands.tsx
      const requestBody = {
        name: selectedBrand.name,
        logo_url: selectedBrand.logo_url || null,
        description_ru: selectedBrand.description_ru || null,
        description_fr: selectedBrand.description_fr || null,
        description_en: selectedBrand.description_en || null,
        hide_name: hideName,
      ...
      if (data.success) {
        setSuccess('Настройки бренда успешно сохранены');
        setTimeout(() => setSuccess(''), 3000);
        
        // Update brand in list
        setBrands(brands.map(brand => 
          brand.id === selectedBrand.id ? data.data : brand
        ));
        setSelectedBrand(data.data);
        
        // Refresh brands list to include any new brands
        fetchBrands();
      } else {
        setError(data.error || 'Failed to save brand settings');
      }
    } catch (err) {
                ...
                >
                  {brands.map((brand) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left text-sm hover:bg-[#FF6B00]/10 transition-colors ${textClass} ${selectedBrand?.id === brand.id ? 'bg-[#FF6B00]/20' : ''}`}
                    >
                      {brand.name}
                    </button>
                  ))}
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <label htmlFor="hideName" className={`text-sm ${textClass}`}>
                Не показывать название на плитке (логотип будет "парить")
              </label>
            </div>
...
export default AdminSettingsBrandsCustomization;
