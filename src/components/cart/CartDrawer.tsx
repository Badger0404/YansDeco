      ...
      {isCartOpen && (
        <>
          ...
          <motion.div
            ...
            className={`fixed right-0 top-0 bottom-0 w-full max-w-[100%] sm:max-w-md z-[70] ${bgClass} shadow-2xl flex flex-col`}
          >
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className={`text-sm ${mutedClass} mt-2 mb-6`}>{t('cart.addItems')}</p>
                  <button
                    onClick={() => {
                      setIsCartOpen(false);
                      navigate('/catalogue');
                    }}
                    className="bg-[#FF6B00] text-black px-8 py-3 rounded-xl font-bold uppercase tracking-wide hover:bg-[#FF8533] transition-colors"
                  >
                ...
                <div className="space-y-4">
                  {items.map((item) => (
                    <motion.div
                      ...
                      className={`p-3 sm:p-4 rounded-xl border ${borderClass}`}
                    >
                      <div className="flex gap-3 sm:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className={`font-bold italic text-[11px] sm:text-sm uppercase tracking-wide truncate ${textClass}`}>
                            {getItemName(item)}
                          </h3>
                          <p className={`text-[9px] sm:text-xs ${mutedClass} mt-0.5`}>
                            Réf: {item.sku}
                          </p>
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-dashed border-gray-300 dark:border-gray-700">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className={`p-1.5 sm:p-2.5 rounded-lg transition-colors ${isLight ? 'bg-gray-100 hover:bg-gray-200' : 'bg-white/10 hover:bg-white/20'}`}
                          >
            ...
            {items.length > 0 && (
              <div className={`p-4 border-t ${borderClass} bg-inherit`}>
                ...
                <button
                  ...
                  className="w-full bg-[#FF6B00] text-black py-4 text-sm font-bold uppercase tracking-wider rounded-xl hover:bg-[#FF8533] transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-[#FF6B00]/20"
                >
